...
import {
  FiActivity, FiBarChart2, FiClock, FiDownload, FiEye, FiEyeOff, FiExternalLink, FiLock, FiSettings, FiGrid,
...
export default function AdminPage() {
  ...
  const [menuHidden, setMenuHidden] = useState(localStorage.getItem("admin_menu_hidden") === "yes");
  ...
  const loadCommandCenter = async () => {
    try {
      const res = await fetch(`${API_URL}/api/admin/command-center`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const clearVisitors = async () => {
    ...
    await fetch(`${API_URL}/api/admin/visitors`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    loadDashboard();
  ...
  const visitors = useMemo(() => {
    return (data?.recentVisitors || []).filter((v) =>
      `${v.ip} ${v.page} ${v.browser} ${v.os} ${v.device} ${v.city} ${v.country} ${v.isp}`
        .toLowerCase()
  ...
  const shellClass = isDark ? "bg-[#020617] text-white" : "bg-[#f6f8ff] text-slate-950";
      ...
      <main className={`p-4 md:p-8 ${menuHidden ? "" : "lg:ml-72"}`}>
        <header className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              {menuHidden ? <FiEye /> : <FiEyeOff />} {menuHidden ? "Show Menu" : "Hide Menu"}
            </button>

            <button
              onClick={toggleTheme}
              className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black shadow-sm"
            >
        ...
        {active === "Command Center" && (
          <section className="mt-6 grid gap-6 xl:grid-cols-3">
            <Card title="Control Panel" desc="Core admin actions" icon={<FiShield />}>
              <div className="grid gap-3">
                <button onClick={() => updateCommand({ maintenanceMode: !command?.maintenanceMode, action: "Maintenance mode toggled" })} className="rounded-2xl bg-slate-100 px-5 py-4 font-black">
                  Maintenance Mode: {command?.maintenanceMode ? "ON" : "OFF"}
                </button>
                <button onClick={() => updateCommand({ liveTracking: !command?.liveTracking, action: "Live tracking toggled" })} className="rounded-2xl bg-slate-100 px-5 py-4 font-black">
                  Live Tracking: {command?.liveTracking !== false ? "ON" : "OFF"}
                </button>
                <button onClick={loadDashboard} className="rounded-2xl bg-slate-950 px-5 py-4 font-black text-white">
                  Force Refresh
                </button>
              </div>
            </Card>

            <Card title="Export Center" desc="Download data" icon={<FiDownload />}>
              <div className="grid gap-3">
                <button onClick={exportVisitorsCSV} className="rounded-2xl bg-slate-950 px-5 py-4 font-black text-white">Export Visitors CSV</button>
                <button onClick={exportMessagesCSV} className="rounded-2xl bg-slate-950 px-5 py-4 font-black text-white">Export Messages CSV</button>
              </div>
            </Card>

            <Card title="Danger Zone" desc="Permanent actions" icon={<FiTrash2 />}>
              <div className="grid gap-3">
                <button onClick={clearVisitors} className="rounded-2xl bg-red-50 px-5 py-4 font-black text-red-600">Clear All Visitors</button>
                <button onClick={clearMessages} className="rounded-2xl bg-red-50 px-5 py-4 font-black text-red-600">Delete All Messages</button>
              </div>
            </Card>
          </section>
        )}
        ...
        {active === "Settings" && (
          <section className="mt-6 grid gap-6 xl:grid-cols-2">
            <Card title="Change Admin Password" desc="Update admin key" icon={<FiLock />}>
              <input value={newPassword} onChange={(e) => setNewPassword(e.target.value)} type="password" placeholder="New admin password" className="w-full rounded-2xl border border-slate-200 px-5 py-4 outline-none" />
              <button onClick={changePassword} className="mt-4 rounded-2xl bg-slate-950 px-5 py-4 font-black text-white">Update Password</button>
            </Card>
            <Card title="Quick Controls" desc="Menu, theme and portfolio" icon={<FiSettings />}>
              <div className="grid gap-3">
                <button onClick={toggleMenu} className="rounded-2xl bg-slate-100 px-5 py-4 font-black">{menuHidden ? "Show Sidebar" : "Hide Sidebar"}</button>
                <button onClick={toggleTheme} className="rounded-2xl bg-slate-100 px-5 py-4 font-black">{theme === "dark" ? "Light Mode" : "Dark Mode"}</button>
                <button onClick={() => window.open("https://naitiksoni1417.netlify.app", "_blank")} className="rounded-2xl bg-slate-950 px-5 py-4 font-black text-white">Open Portfolio</button>
              </div>
            </Card>
          </section>
        )}
...
function VisitorDetailModal({ visitor, onClose }) {
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
      <div className="w-full max-w-3xl rounded-[2rem] bg-white p-6 text-slate-950 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-black text-blue-600">VISITOR PROFILE</p>
            <h2 className="mt-1 text-3xl font-black">{visitor.ip || "Unknown IP"}</h2>
            <p className="mt-1 text-sm font-semibold text-slate-500">
              {visitor.isReturning ? "Returning Visitor" : "New Visitor"} • {visitor.visits || 1} visits
            </p>
          </div>

          <button onClick={onClose} className="rounded-full bg-slate-100 p-3 text-xl hover:bg-slate-200">
            <FiX />
          </button>
        </div>

        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <VisitorInfo icon={<FiMapPin />} label="Location" value={`${visitor.city || "Unknown"}, ${visitor.region || ""}, ${visitor.country || "Unknown"}`} />
          <VisitorInfo icon={<FiGlobe />} label="ISP" value={visitor.isp || "Unknown"} />
          <VisitorInfo icon={<FiMonitor />} label="Device / OS" value={`${visitor.device || "Desktop"} • ${visitor.os || "Unknown"}`} />
          <VisitorInfo icon={<FiEye />} label="Browser" value={visitor.browser || "Unknown"} />
          <VisitorInfo icon={<FiActivity />} label="Page Visited" value={visitor.page || "/"} />
          <VisitorInfo icon={<FiClock />} label="Last Seen" value={visitor.createdAt ? new Date(visitor.createdAt).toLocaleString() : "Unknown"} />
        </div>

        <div className="mt-5 rounded-3xl bg-slate-950 p-5 text-white">
          <p className="text-xs font-black text-slate-400">USER AGENT</p>
...
function VisitorInfo({ icon, label, value }) {
  return (
...
function Messages({ messages, exportMessagesCSV, updateMessageStatus, deleteMessage }) {
  return (
    <section className="mt-6">
      <div className="mb-5 flex flex-col gap-4 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-black">Messages Inbox</h2>
          <p className="text-sm font-semibold text-slate-500">Read, manage and export contact messages</p>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {messages.length === 0 && (
          <div className="rounded-[2rem] border border-slate-200 bg-white p-8 font-semibold text-slate-500">
            No messages yet
          </div>
        )}

        {messages.map((m) => (
          <div key={m._id} className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-black">{m.name}</h3>
                <p className="text-sm font-semibold text-slate-500">{m.email}</p>
              </div>
            ...
            <p className="mt-5 leading-relaxed text-slate-700">{m.message}</p>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button
                ...
                className="flex items-center gap-2 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-700"
              >
                {m.status === "Unread" ? <FiCheckCircle /> : <FiCircle />}
                Mark {m.status === "Unread" ? "Read" : "Unread"}
              </button>

              <button
                onClick={() => deleteMessage(m._id)}
                className="flex items-center gap-2 rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600"
              >
                <FiTrash2 /> Delete
              </button>
            </div>
