...
function NSAIChat({ data, security, token, loadDashboard, loadAdvancedAnalytics, theme }) {
  ...
  const buildChartFromQuestion = (q) => {
    const lower = q.toLowerCase();

    if (!/(chart|graph|visual|bar|line|pie|trend)/i.test(lower)) return null;

    if (/traffic|views|growth|trend|daily|today/i.test(lower)) {
      return {
        type: "line",
        title: "Traffic Trend",
        subtitle: "Daily views from dashboard analytics",
        labelKey: "label",
        valueKey: "views",
        data: (data?.dailyViews || []).map((d) => ({ label: d._id, views: d.views || 0 })),
      };
    }

    if (/city|cities/i.test(lower)) {
      return {
        type: "bar",
        title: "Top Cities",
        subtitle: "Visitor distribution by city",
        labelKey: "label",
        valueKey: "count",
        data: (data?.cities || []).map((d) => ({ label: d._id || "Unknown", count: d.count || 0 })),
      };
    }

    if (/country|countries/i.test(lower)) {
      return {
        type: "bar",
        title: "Top Countries",
        ...
        labelKey: "label",
        valueKey: "count",
        data: (data?.countries || []).map((d) => ({ label: d._id || "Unknown", count: d.count || 0 })),
      };
    }

    if (/browser|chrome|safari|firefox|edge/i.test(lower)) {
      return {
        type: "bar",
        title: "Browser Breakdown",
        subtitle: "Visitors grouped by browser",
        labelKey: "label",
        valueKey: "count",
        data: (data?.browsers || []).map((d) => ({ label: d._id || "Unknown", count: d.count || 0 })),
      };
    }

    if (/os|operating|windows|mac|android|ios/i.test(lower)) {
      return {
        type: "bar",
        title: "Operating System Breakdown",
        subtitle: "Visitors grouped by OS",
        labelKey: "label",
        valueKey: "count",
        data: (data?.osStats || []).map((d) => ({ label: d._id || "Unknown", count: d.count || 0 })),
      };
    }

    if (/device|desktop|mobile|tablet/i.test(lower)) {
      return {
        type: "bar",
        title: "Device Split",
        subtitle: "Visitors grouped by device type",
        labelKey: "label",
        valueKey: "count",
        data: (data?.devices || []).map((d) => ({ label: d._id || "Unknown", count: d.count || d.value || 0 })),
      };
    }

    if (/page|pages|route|top page/i.test(lower)) {
      return {
        type: "bar",
        title: "Top Pages",
        subtitle: "Most visited portfolio pages",
        labelKey: "label",
        valueKey: "count",
        data: (data?.topPages || []).map((d) => ({ label: d._id || "/", count: d.count || 0 })),
      };
    }

    if (/message|messages|unread/i.test(lower)) {
      return {
        type: "bar",
        title: "Messages Overview",
        subtitle: "Total vs unread messages",
        labelKey: "label",
        valueKey: "count",
        data: [
          { label: "Total", count: data?.totalMessages || 0 },
          { label: "Unread", count: data?.unreadMessages || 0 },
          { label: "Read", count: Math.max(0, (data?.totalMessages || 0) - (data?.unreadMessages || 0)) },
        ],
      };
    }

    if (/security|failed|login|suspicious|ip/i.test(lower)) {
      return {
        type: "bar",
        title: "Security Overview",
        subtitle: "Failed logins and suspicious IP indicators",
        labelKey: "label",
        valueKey: "count",
        data: [
          { label: "Failed Logins", count: security?.failedLoginCount || 0 },
          { label: "Blocked IPs", count: security?.blockedIps?.length || 0 },
          { label: "Suspicious IPs", count: security?.suspiciousIps?.length || 0 },
        ],
      };
    }
  ...
  const askAI = async (q = input) => {
    ...
    const chart = buildChartFromQuestion(q);
    ...
    if (chart) {
      setMessages((prev) => [
        ...
        {
          role: "ai",
          text: `Here is your ${chart.title.toLowerCase()} based on live admin data.`,
          chart,
        }
      ]);
    }
  ...
  return (
    <section className={`relative -mt-4 min-h-[calc(100vh-40px)] overflow-hidden rounded-[34px] p-5 shadow-2xl ${
      ...
      <div className={`absolute inset-0 ${
        isDark
          ? "bg-[radial-gradient(circle_at_20%_10%,rgba(34,211,238,0.22),transparent_28%),radial-gradient(circle_at_85%_20%,rgba(168,85,247,0.24),transparent_30%),radial-gradient(circle_at_50%_90%,rgba(59,130,246,0.16),transparent_34%)]"
          : "bg-[radial-gradient(circle_at_20%_10%,rgba(34,211,238,0.14),transparent_28%),radial-gradient(circle_at_85%_20%,rgba(168,85,247,0.12),transparent_30%),radial-gradient(circle_at_50%_90%,rgba(59,130,246,0.08),transparent_34%)]"
      }`} />
      ...
      <div className="relative z-10 h-full">
        <div className={`flex min-h-[760px] flex-col overflow-hidden rounded-[30px] backdrop-blur-2xl ${
          ...
          <div className={`shrink-0 border-b p-5 ${isDark ? "border-white/10" : "border-slate-200/70"}`}>
          ...
          <div className={`shrink-0 border-b px-5 py-3 ${isDark ? "border-white/10" : "border-slate-200/70"}`}>
            <div className="mb-3 flex items-center justify-between">
              <h3 className={`text-sm font-black uppercase tracking-[0.25em] ${isDark ? "text-emerald-300" : "text-emerald-600"}`}>
                Premium Questions
              </h3>
              <span className={`text-xs font-bold ${isDark ? "text-slate-400" : "text-slate-500"}`}>Scroll →</span>
            </div>

            <div className="flex gap-3 overflow-x-auto pb-1 ns-ai-scroll">
              {prompts.map((q) => (
                <button
                  key={q}
                  onClick={() => askAI(q)}
                  className={`shrink-0 rounded-full border px-5 py-3 text-sm font-black transition hover:border-cyan-300/60 hover:bg-cyan-300/10 ${
                    isDark
          ...
          <div className="min-h-0 flex-1 space-y-6 overflow-y-auto px-6 py-5 ns-ai-scroll">
            {messages.map((m, i) => (
                    ...
                    <div className="mt-5 rounded-2xl border border-cyan-300/20 bg-black/20 p-4">
                      ...
                      <div className="space-y-3">
                        {(m.chart.data || []).length === 0 && (
                          <p className="text-sm font-bold text-slate-400">No chart data available yet.</p>
                        )}

                        {(m.chart.data || []).map((item, idx) => {
                          const valueKey = m.chart.valueKey || "count";
                          const labelKey = m.chart.labelKey || "label";
                          const max = Math.max(...(m.chart.data || []).map((x) => x[valueKey] || 0), 1);
                          const width = Math.max(8, ((item[valueKey] || 0) / max) * 100);

                          return (
                            <div key={idx}>
                              <div className="mb-1 flex justify-between gap-3 text-xs font-black">
                                <span className="max-w-[220px] truncate text-slate-300">{item[labelKey]}</span>
                                <span className="text-emerald-300">{item[valueKey]}</span>
                              </div>
                              <div className="h-3 overflow-hidden rounded-full bg-white/10">
                                <div
                                  className="h-full rounded-full bg-gradient-to-r from-cyan-400 via-blue-400 to-violet-500 shadow-[0_0_18px_rgba(34,211,238,0.25)]"
                                  style={{ width: `${width}%` }}
                                />
                              </div>
                            </div>
            ...
            {thinking && (
              <div className="flex items-center gap-4">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/[0.07] px-5 py-4 text-sm font-black text-emerald-300">
          ...
          <div className={`shrink-0 px-6 pb-6 pt-3 ${
            ...
            <div className={`relative mx-auto flex max-w-[980px] items-center gap-3 rounded-[999px] border p-2 ${
              isDark
                ? "border-cyan-300/50 bg-[#07111f]/95 shadow-[0_0_35px_rgba(34,211,238,0.22),inset_0_0_30px_rgba(255,255,255,0.04)]"
                : "border-cyan-300/50 bg-white shadow-[0_22px_70px_rgba(15,23,42,0.16)]"
            }`}>
              ...
              <input
                ...
                className={`relative z-10 flex-1 rounded-full bg-transparent px-6 py-4 text-[15px] font-bold outline-none placeholder:text-slate-400 ${
              ...
              <button
                onClick={() => askAI()}
                disabled={thinking}
                className="relative z-10 rounded-full bg-gradient-to-br from-cyan-300 via-blue-400 to-violet-500 px-8 py-4 font-black text-white shadow-[0_0_28px_rgba(34,211,238,0.35)] transition hover:scale-105 disabled:opacity-50"
              >
                Send
              </button>
            </div>
