      ...
      <main className={`p-4 md:p-8 ${menuHidden || active === "NS.ai" ? "" : "lg:ml-72"}`}>
        ...
        {active === "SOC Panel" && (
          <section className="mt-6 space-y-6">
            <div className="rounded-[2.5rem] border border-slate-200 bg-slate-950 p-7 text-white shadow-2xl">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <p className="text-xs font-black tracking-[0.35em] text-cyan-300">NS.ai SECURITY OPERATIONS CENTER</p>
                  <h2 className="mt-3 text-4xl font-black">SOC Panel</h2>
                  <p className="mt-2 max-w-2xl text-sm font-bold text-slate-400">
                    Real-time DoS spike detection, API abuse monitoring, suspicious traffic analysis and auto-block visibility.
                  </p>
                </div>

                <div className={`rounded-[2rem] px-6 py-5 text-center ${
                  (socData?.threatLevel || "LOW") === "CRITICAL"
                    ? "bg-red-500/20 text-red-200"
                    : (socData?.threatLevel || "LOW") === "HIGH"
                    ? "bg-orange-500/20 text-orange-200"
                    : (socData?.threatLevel || "LOW") === "MEDIUM"
            ...
            <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                ["Active IPs", socData?.activeIps || 0, "Tracked in memory"],
                ["Blocked IPs", socData?.blockedIps?.length || 0, "Auto-blocked"],
                ["Events", socData?.events?.length || 0, "Recent SOC events"],
            ...
            <div className="grid gap-6 xl:grid-cols-3">
              <Card className="xl:col-span-2" title="Top Traffic IPs" desc="Highest request volume in last 30 seconds" icon={<FiShield />}>
                    ...
                    <tbody>
                      {(socData?.topIps || []).map((ip) => (
                        <tr key={ip.ip} className="border-t border-slate-100">
                          <td className="py-4 font-black">{ip.ip}</td>
                          <td className="font-bold">{ip.requestsLast30s}</td>
                          <td className="font-bold">{ip.score}</td>
                          <td>
                            <span className={`rounded-full px-3 py-1 text-xs font-black ${
                              ip.blocked ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"
                            }`}>
                              {ip.blocked ? "Blocked" : "Allowed"}
                            </span>
                          </td>
                          <td className="flex gap-2 py-3">
                            {ip.blocked ? (
                              <button onClick={() => unblockSocIp(ip.ip)} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white">
                                Unblock
                              </button>
                            ) : (
                              <button onClick={() => blockSocIp(ip.ip)} className="rounded-xl bg-red-600 px-3 py-2 text-xs font-black text-white">
                                Block IP
                              </button>
                            )}
                          </td>
              ...
              <Card title="Emergency Lockdown" desc="Strict SOC firewall mode" icon={<FiActivity />}>
                <button
                  onClick={toggleSocLockdown}
                  className={`mb-4 w-full rounded-2xl px-5 py-4 font-black text-white shadow-lg ${
                    socData?.emergencyLockdown ? "bg-emerald-600" : "bg-red-600"
                  }`}
                >
                  {socData?.emergencyLockdown ? "Disable Lockdown" : "Enable Emergency Lockdown"}
                </button>
                <div className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
                  {socData?.emergencyLockdown
                    ? "Strict mode is ON. Suspicious spikes are blocked faster."
                    : "Normal mode is ON. SOC is monitoring safely."}
                </div>
              </Card>

              <Card title="NS.ai Recommendation" desc="Defensive guidance" icon={<FiActivity />}>
                <div className="rounded-[2rem] bg-slate-950 p-5 text-white">
                  <p className="text-sm font-bold leading-relaxed text-slate-300">
                    {socData?.recommendation || "Portfolio traffic looks stable. NS.ai SOC is actively monitoring."}
                  </p>
                </div>
              </Card>
            </div>

            <Card title="Security Timeline" desc="Recent SOC detection events" icon={<FiShield />}>
              <div className="max-h-[420px] overflow-y-auto">
                {(socData?.events || []).length === 0 && (
                  <p className="rounded-2xl bg-slate-50 p-5 font-bold text-slate-500">No SOC events yet. System is clean.</p>
                )}
                ...
                {(socData?.events || []).map((e, i) => (
                  <div key={i} className="mb-3 rounded-2xl border border-slate-200 bg-white p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <p className="font-black text-slate-950">{e.type}</p>
                        <p className="mt-1 text-sm font-bold text-slate-500">{e.reason}</p>
                      </div>
                      <span className={`rounded-full px-3 py-1 text-xs font-black ${
                    ...
                    <p className="mt-3 text-xs font-bold text-slate-400">
                      IP: {e.ip} • Path: {e.path} • {new Date(e.createdAt).toLocaleString()}
                    </p>
                  </div>
                ))}
              </div>
